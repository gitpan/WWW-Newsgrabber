...
sub _init(){
	...
	my $self 	= shift;
	my $Url		= $self->{'url'} || die "$self->_init()  no url to scan given!\n";
...
sub HtmlLinkExtractor(){
	...
	my $self			= shift;
	my $Regex			= $self->{'regex'} || warn "$self->_init()  no regex given!\n";
	my $HashRef			= shift;
...
use WWW::Newsgrabber;
my $obj           =  WWW::Newsgrabber->new(
		url =>	'http://www.newsitedoesntexists.com',
                regex => '\.html'
);
